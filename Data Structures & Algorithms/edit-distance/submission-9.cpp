...
public:
    ...
    int helper(string word1, string word2, int ptr1, int ptr2, vector<vector<int>>&dp){
        if(ptr1 == word1.size()) return word2.size() - ptr2; 
        if(ptr2 == word2.size()) return word1.size() - ptr1; 
        if(dp[ptr1][ptr2] != -1) return dp[ptr1][ptr2];

        if(word1[ptr1] == word2[ptr2]) return dp[ptr1][ptr2] = helper(word1, word2, ptr1+1, ptr2+1, dp);
        int c2 = 1 + helper(word1, word2, ptr1, ptr2+1, dp);    
        int c3 = 1 + helper(word1, word2, ptr1+1, ptr2, dp);    
        int c4 = 1 + helper(word1, word2, ptr1+1, ptr2+1, dp);  
